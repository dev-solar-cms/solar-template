 ...
 *          solar_template_apply_catalog_filters_to_main_query() already filters that URL), then
 *          intercepted by assets/js/catalog.js to sync the filter bar's own controls and submit
 *          via AJAX instead of navigating.
 *
 * @package Solar_Template
 * @var array $args {
 *     @type array|null  $filters  See solar_template_get_active_catalog_filter_chips()'s $filters
 *                                   parameter. Only ever passed by the AJAX handler.
...
$chips = solar_template_get_active_catalog_filter_chips( $active_filters_args['filters'], $active_filters_args['base_url'] );
?>
<div class="catalog-active-filters<?php echo ! empty( $chips ) ? ' catalog-active-filters--has-chips' : ''; ?>" id="catalog-active-filters">
	<?php if ( ! empty( $chips ) ) : ?>
		...
		<a
			href="<?php echo esc_url( solar_template_catalog_clear_filters_url( $active_filters_args['base_url'] ) ); ?>"
