...
function solar_template_catalog_columns(): int {
	return solar_template_load_autoloader()
...
function solar_template_catalog_result_count_label( int $count ): string {
	return solar_template_load_autoloader()
		? \Solar_Template\Catalog\CatalogOptions::result_count_label( $count )
		: '';
}
 ...
 * @param string|null $base_url See \Solar_Template\Catalog\CatalogFilters::url()'s $base_url
 *                                parameter — the "next page" link is built against it the same way.
...
function solar_template_get_catalog_load_more_config( \WP_Query $query, ?string $base_url = null ): ?array {
	return solar_template_load_autoloader()
		? \Solar_Template\Catalog\CatalogPagination::load_more_config( $query, $base_url )
		: null;
}
...
function solar_template_catalog_color_attribute_slug(): string {
	return solar_template_load_autoloader()
		? \Solar_Template\Catalog\CatalogOptions::color_attribute_slug()
		: 'pa_color';
}
...
function solar_template_catalog_size_attribute_slug(): string {
	return solar_template_load_autoloader()
		? \Solar_Template\Catalog\CatalogOptions::size_attribute_slug()
		: 'pa_size';
}
...
function solar_template_get_catalog_category_options(): array {
	return solar_template_load_autoloader()
		? \Solar_Template\Catalog\CatalogOptions::category_options()
		: array();
}
...
function solar_template_get_catalog_attribute_options( string $taxonomy ): array {
	return solar_template_load_autoloader()
		? \Solar_Template\Catalog\CatalogOptions::attribute_options( $taxonomy )
		: array();
}
 ...
 * \Solar_Template\Catalog\CatalogFilters::query_args()), and checking more than one option here simply ORs
...
function solar_template_get_catalog_rating_options(): array {
	return solar_template_load_autoloader()
		? \Solar_Template\Catalog\CatalogOptions::rating_options()
		: array();
}
...
function solar_template_get_catalog_price_bounds(): array {
	return solar_template_load_autoloader()
		? \Solar_Template\Catalog\CatalogOptions::price_bounds()
		: array(
			'min' => 0.0,
			'max' => 0.0,
		);
...
function solar_template_catalog_filters_position(): string {
	return solar_template_load_autoloader()
		? \Solar_Template\Catalog\CatalogOptions::filters_position()
		: 'sticky-top';
}
...
function solar_template_get_catalog_sort_options(): array {
	return solar_template_load_autoloader()
		? \Solar_Template\Catalog\CatalogOptions::sort_options()
		: array();
}
...
function solar_template_catalog_filter_group_is_active( array $group ): bool {
	return solar_template_load_autoloader()
		? \Solar_Template\Catalog\CatalogFilters::group_is_active( $group )
		: false;
}
 ...
 *
 * @return array See \Solar_Template\Catalog\CatalogFilters::sanitize()'s return type.
 */
function solar_template_get_active_catalog_filters(): array {
	return solar_template_load_autoloader()
		? \Solar_Template\Catalog\CatalogFilters::active()
		: array();
}

/**
 * Returns the current catalog view's URL with every filter query arg removed, used as the
 * "Clear all" link and as the base URL for \Solar_Template\Catalog\CatalogFilters::url().
 *
 * @param string|null $base_url See \Solar_Template\Catalog\CatalogFilters::url()'s $base_url
 *                                parameter.
 * @return string
...
function solar_template_catalog_clear_filters_url( ?string $base_url = null ): string {
	return solar_template_load_autoloader()
		? \Solar_Template\Catalog\CatalogFilters::clear_url( $base_url )
		: (string) ( $base_url ?? '' );
}
 ...
 *                                request URL (via \Solar_Template\Catalog\CatalogFilters::active())
...
function solar_template_get_active_catalog_filter_chips( ?array $filters = null, ?string $base_url = null ): array {
	return solar_template_load_autoloader()
		? \Solar_Template\Catalog\CatalogFilters::active_chips( $filters, $base_url )
		: array();
}
...
function solar_template_apply_catalog_filters_to_main_query( \WP_Query $query ): void {
	if ( solar_template_load_autoloader() ) {
		\Solar_Template\Catalog\CatalogController::apply_filters_to_main_query( $query );
	}
}
 ...
 * validates the nonce, then delegates to the catalog controller, which sanitizes the submitted
 * filters, runs them as the page's main product query (so WooCommerce's own visibility/stock/
 * ordering logic — only ever applied to the main query — still applies, exactly as on a plain
 * page load), and responds with the re-rendered results markup (template-parts/catalog-results.php)
 * the front-end (assets/js/catalog.js) swaps into the page.
 *
 * @return void
 */
function solar_template_handle_catalog_filter_request(): void {
	...
	\Solar_Template\Catalog\CatalogController::handle_filter_request();
}
...
function solar_template_enqueue_catalog_script(): void {
	if ( solar_template_load_autoloader() ) {
		\Solar_Template\Catalog\CatalogController::enqueue_script();
	}
}
